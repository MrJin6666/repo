...
import com.goboosoft.system.sys.dao.SysDictDao;
...
public class SysDictServiceImpl extends BaseServiceImpl<SysDictDao, SysDictEntity> implements SysDictService {
     ...
     */
    @Override
    public List<SysDictEntity> getEntityByDictType(String dictType) {
        QueryWrapper<SysDictEntity> wr = new QueryWrapper<>();
        wr.eq("dict_type", dictType);
        wr.orderByAsc("sort");
        return baseDao.selectList(wr);
    }

    @Override
    public List<SysDictEntity> getEntityByDictTypeAndPid(String dictType, Long pid) {
        QueryWrapper<SysDictEntity> wr = new QueryWrapper<>();
        wr.eq("dict_type", dictType);
        wr.eq("pid", pid);
        wr.orderByAsc("sort");
